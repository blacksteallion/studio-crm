...
<div class="max-w-4xl mx-auto">
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        <form action="{{ route('customers.update', $customer->id) }}" method="POST" class="p-6" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="col-span-1 md:col-span-2 border-t border-gray-100 pt-6 mt-2">
                    ...
                    <textarea name="remarks" rows="3" 
                        class="w-full rounded-xl border py-3 px-5 outline-none transition 
                        @error('remarks') border-red-500 bg-red-50 text-red-900 placeholder-red-400 focus:border-red-500 focus:ring-red-500 @else border-gray-200 bg-gray-50 text-slate-800 focus:border-blue-500 focus:bg-white placeholder-gray-400 @enderror">{{ old('remarks', $customer->remarks ?? '') }}</textarea>
                    @error('remarks')
                        <p class="mt-1 text-xs font-bold text-red-500 flex items-center gap-1"><i class="fas fa-exclamation-circle"></i> {{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <div class="flex items-center gap-3 p-4 rounded-xl border border-gray-100 bg-gray-50">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="status" value="1" {{ old('status', $customer->status) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="border-t border-gray-100 pt-6 flex justify-end gap-3">
                ...
                <button type="submit" class="rounded-xl bg-blue-600 py-3 px-8 font-bold text-white hover:bg-blue-700 transition shadow-md">Update Customer</button>
            </div>
        </form>
    </div>
</div>
