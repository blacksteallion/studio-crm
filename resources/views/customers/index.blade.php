...
<x-card title="Customer List">
    ...
    <x-slot name="action">
        <div class="flex items-center justify-end gap-1.5 sm:gap-2">
            ...
            <form action="{{ route('customers.index') }}" method="GET" class="hidden md:block relative w-64 search-form">
                @foreach(request()->only(['f_name', 'f_business', 'f_email', 'f_mobile', 'f_status']) as $key => $value)
            ...
            <div class="hidden md:block">
                <x-button href="{{ route('customers.create') }}" icon="fas fa-plus" class="!h-9 !py-0 flex items-center">
                    Add Customer
                </x-button>
            </div>
            <a href="{{ route('customers.create') }}" class="md:hidden h-9 w-9 flex items-center justify-center rounded-lg bg-blue-600 text-white shadow-sm hover:bg-blue-700 transition" title="Add Customer">
    ...
    <div x-data="{ searchOpen: {{ request('search') ? 'true' : 'false' }} }"
        ...
        <form action="{{ route('customers.index') }}" method="GET" class="relative w-full search-form">
            @foreach(request()->only(['f_name', 'f_business', 'f_email', 'f_mobile', 'f_status']) as $key => $value)
                <input type="hidden" name="{{ $key }}" value="{{ $value }}">
            @endforeach
    ...
    <div x-data="{ show: {{ request()->hasAny(['f_name', 'f_business', 'f_email', 'f_mobile', 'f_status']) ? 'true' : 'false' }} }"
        ...
        <form action="{{ route('customers.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <div>
                    <label class="block text-xs font-bold text-black uppercase mb-2">Customer Name</label>
                    <input type="text" name="f_name" value="{{ request('f_name') }}" class="w-full rounded-xl border border-gray-200 bg-white py-2.5 px-4 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-black placeholder-gray-400 shadow-sm" placeholder="Name">
                </div>
                <div>
                    <label class="block text-xs font-bold text-black uppercase mb-2">Business Name</label>
                    <input type="text" name="f_business" value="{{ request('f_business') }}" class="w-full rounded-xl border border-gray-200 bg-white py-2.5 px-4 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-black placeholder-gray-400 shadow-sm" placeholder="Company">
                </div>
                <div>
            ...
            <div class="flex justify-end gap-3 mt-6">
                <a href="{{ route('customers.index') }}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white px-5 py-2.5 text-sm font-bold text-black shadow-sm hover:bg-gray-50 transition">
    ...
    <div class="hidden md:block">
        <x-table :headers="['#', 'Photo', 'Customer', 'Contact', 'Status', 'Actions']">
            ...
            <tr class="hover:bg-gray-50 transition duration-200">
                ...
                <td class="px-6 py-4 w-16 align-middle">
                    <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-black font-bold text-xs border border-gray-200 shadow-sm">
                        {{ substr($customer->name, 0, 1) }}
                    </div>
                </td>
                
                <td class="px-6 py-4 align-middle">
                    <div class="font-bold text-black text-sm">{{ $customer->business_name ?? 'Individual' }}</div>
                    <div class="text-sm text-black mt-0.5 font-medium">
                        <i class="far fa-user mr-1"></i> {{ $customer->name }}
                    </div>
                </td>
                
                <td class="px-6 py-4 align-middle">
                    <div class="text-sm text-black font-medium flex items-center gap-2">
                        <i class="fas fa-phone-alt text-black opacity-70"></i> {{ $customer->mobile }}
                    </div>
                    @if($customer->email)
                    <div class="text-sm text-black mt-1 flex items-center gap-2">
                        <i class="fas fa-envelope text-black opacity-70"></i> {{ $customer->email }}
                    </div>
                    @endif
                </td>
                ...
                <td class="px-6 py-4 align-middle">
                    ...
                    @else
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold {{ $customer->status ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                ...
                <td class="px-6 py-4 text-right align-middle">
                    <div class="flex justify-end items-center gap-1">
                        <x-button variant="secondary" href="{{ route('customers.show', $customer->id) }}" class="!p-1.5 !rounded-lg !border-0 !shadow-none text-black hover:!bg-gray-100" title="View Profile">
                            <i class="fas fa-eye"></i>
                        </x-button>
                        
                        @can('edit customers')
                        <x-button variant="secondary" href="{{ route('customers.edit', $customer->id) }}" class="!p-1.5 !rounded-lg !border-0 !shadow-none text-blue-500 hover:!bg-blue-50" title="Edit">
                        ...
                        @can('delete customers')
                            @if($customer->inquiries_count > 0 || $customer->bookings_count > 0 || $customer->orders_count > 0)
                                <button type="button" 
                                    onclick="window.dispatchEvent(new CustomEvent('notify', { detail: { type: 'warning', title: 'Action Blocked', message: 'Cannot delete this customer because they have associated records.' } }))"
                                    class="p-1.5 rounded-lg text-red-200 hover:bg-red-50 cursor-not-allowed transition" 
                                    title="Cannot Delete: Associated Records">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                            @else
                                <form id="del-cust-{{ $customer->id }}" action="{{ route('customers.destroy', $customer->id) }}" method="POST" class="hidden">
                                    @csrf @method('DELETE')
                                </form>
                                <x-button variant="secondary" type="button" class="!p-1.5 !rounded-lg !border-0 !shadow-none text-red-500 hover:!bg-red-50" title="Delete"
                                    onclick="confirmDelete('del-cust-{{ $customer->id }}', 'Are you sure you want to delete this customer?')">
                                    <i class="fas fa-trash-alt"></i>
                                </x-button>
                            @endif
                        @endcan
    ...
    <div class="block md:hidden space-y-4 p-4">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center text-black font-bold text-lg border border-gray-200 shadow-sm">
                    {{ substr($customer->name, 0, 1) }}
                </div>
                <div>
                    <div class="font-bold text-gray-900 text-sm">{{ $customer->business_name ?? 'Individual' }}</div>
            ...
            <div class="flex flex-col gap-2 bg-gray-50 p-3 rounded-xl border border-gray-100">
                <div class="text-sm text-gray-700 flex items-center gap-3">
                    <i class="fas fa-phone-alt opacity-50 w-4 text-center"></i> {{ $customer->mobile }}
                </div>
                @if($customer->email)
                <div class="text-sm text-gray-700 flex items-center gap-3">
                    <i class="fas fa-envelope opacity-50 w-4 text-center"></i> {{ $customer->email }}
                </div>
                @endif
            </div>
            ...
            <div class="flex items-center justify-between pt-1">
                ...
                <div class="flex justify-end items-center gap-2">
                    <a href="{{ route('customers.show', $customer->id) }}" class="p-2 rounded-lg bg-gray-50 text-gray-600 hover:text-black border border-gray-200 shadow-sm transition" title="View Profile">
                        <i class="fas fa-eye w-4 h-4 flex items-center justify-center"></i>
                    </a>

                    @can('edit customers')
                    <a href="{{ route('customers.edit', $customer->id) }}" class="p-2 rounded-lg bg-gray-50 text-blue-600 hover:text-blue-800 border border-gray-200 shadow-sm transition" title="Edit">
                    ...
                    @can('delete customers')
                        @if($customer->inquiries_count > 0 || $customer->bookings_count > 0 || $customer->orders_count > 0)
                            <button type="button" 
                                onclick="window.dispatchEvent(new CustomEvent('notify', { detail: { type: 'warning', title: 'Action Blocked', message: 'Cannot delete this customer because they have associated records.' } }))"
                                class="p-2 rounded-lg bg-gray-50 text-red-200 hover:bg-red-50 border border-gray-200 shadow-sm cursor-not-allowed transition" 
                                title="Cannot Delete: Associated Records">
                                <i class="fas fa-trash-alt w-4 h-4 flex items-center justify-center"></i>
                            </button>
                        @else
                            <form id="del-cust-mobile-{{ $customer->id }}" action="{{ route('customers.destroy', $customer->id) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                            <button type="button" class="p-2 rounded-lg bg-gray-50 text-red-600 hover:text-red-800 border border-gray-200 shadow-sm transition" title="Delete"
                                onclick="confirmDelete('del-cust-mobile-{{ $customer->id }}', 'Are you sure you want to delete this customer?')">
                                <i class="fas fa-trash-alt w-4 h-4 flex items-center justify-center"></i>
                            </button>
                        @endif
                    @endcan
...
<script>
    ...
    function confirmExport() {
        ...
        const exportUrl = "{{ route('customers.export') }}?" + urlParams.toString();

        window.dispatchEvent(new CustomEvent('confirm', { 
            detail: { 
                title: 'Export Confirmation', 
                message: 'Do you want to export the current filtered list of customers to Excel?', 
                type: 'info', 
    ...
    function toggleStatus(id) {
        ...
        .then(response => {
            if (!response.ok) {
                ...
                window.dispatchEvent(new CustomEvent('notify', { 
                    detail: { type: 'success', title: 'Success', message: 'Customer status updated.' } 
                }));
            }
        })
        .catch(error => {
