@extends('layouts.app')
@section('header', 'Add Product / Service')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200">
        <div class="border-b border-gray-100 py-4 px-6">
            <h3 class="font-bold text-slate-800 text-lg">New Item Details</h3>
        </div>
        
        <form action="{{ route('product_services.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
            </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="pricing_model" class="w-full appearance-none rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                            <option value="Hourly" {{ old('pricing_model') == 'Hourly' ? 'selected' : '' }}>Hourly</option>
                            <option value="Fixed" {{ old('pricing_model') == 'Fixed' ? 'selected' : '' }}>Fixed</option>
                            <option value="Per Unit" {{ old('pricing_model') == 'Per Unit' ? 'selected' : '' }}>Per Unit</option>
                        </select>
                        <span class="absolute right-4 top-3.5 text-gray-500 pointer-events-none"><i class="fas fa-chevron-down"></i></span>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="price" value="{{ old('price') }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                </div>
                <div>
                    <label class="mb-2 block font-bold text-sm text-slate-700">GST Rate (%) <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <input type="number" step="0.01" name="gst_rate" value="{{ old('gst_rate', 0) }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="3" class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6 flex justify-end gap-3">
                ...
                <button type="submit" class="rounded-xl bg-blue-600 py-3 px-8 font-bold text-white hover:bg-blue-700 transition shadow-md">Save Item</button>
            </div>
        </form>
    </div>
</div>
