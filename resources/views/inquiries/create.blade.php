@extends('layouts.app')
@section('header', 'Add New Inquiry')
...
<div class="max-w-5xl mx-auto">
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div class="border-b border-gray-100 py-4 px-6">
            <h3 class="font-bold text-slate-800 text-lg">New Inquiry Details</h3>
        </div>
        
        <form action="{{ route('inquiries.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white @error('name') !border-red-500 !bg-red-50 @enderror">
                </div>
                <div>
                    <label class="mb-2 block font-bold text-sm text-slate-700">Mobile Number <span class="text-red-500">*</span></label>
                    <input type="text" name="mobile" value="{{ old('mobile') }}" required class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white @error('mobile') !border-red-500 !bg-red-50 @enderror">
                </div>
                <div>
                    <label class="mb-2 block font-bold text-sm text-slate-700">Email Address</label>
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                </div>
                <div>
                    <label class="mb-2 block font-bold text-sm text-slate-700">Business / Company Name</label>
                    <input type="text" name="business_name" value="{{ old('business_name') }}" class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                </div>
                ...
                <div class="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="budget" value="{{ old('budget') }}" class="w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white" placeholder="0.00">
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="primary_date" value="{{ old('primary_date') }}" required class="date-input w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white @error('primary_date') !border-red-500 !bg-red-50 @enderror">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="alternate_date" value="{{ old('alternate_date') }}" class="date-input w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                            <span class="absolute right-4 top-3.5 text-gray-400 pointer-events-none"><i class="fas fa-calendar-alt"></i></span>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="from_time" value="{{ old('from_time') }}" required class="time-input w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="to_time" value="{{ old('to_time') }}" required class="time-input w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                            <span class="absolute right-4 top-3.5 text-gray-400 pointer-events-none"><i class="fas fa-clock"></i></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-2 mt-4" x-data="itemManager()">
                        ...
                        <template x-for="(item, index) in items" :key="index">
                            <div class="flex flex-col md:flex-row gap-3 md:items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                                ...
                                <div class="w-full md:w-8 text-right">
                                    <button type="button" @click="items.splice(index, 1)" class="text-red-400 hover:text-red-600"><i class="fas fa-trash-alt"></i></button>
                                </div>
                            </div>
                        </template>
                    ...
                    <div class="flex justify-between items-center mt-3 bg-blue-50 p-3 rounded-lg border border-blue-100" x-show="items.length > 0">
                        ...
                        <input type="hidden" name="calculated_budget" :value="totalEstimate.toFixed(2)"> 
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="relative">
                            <select name="assigned_staff_id" class="w-full appearance-none rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
                                ...
                                @foreach($staffMembers as $staff)
                                    <option value="{{ $staff->id }}" {{ old('assigned_staff_id') == $staff->id ? 'selected' : '' }}>{{ $staff->name }}</option>
                                @endforeach
                            </select>
                            <span class="absolute right-4 top-3.5 text-gray-500 pointer-events-none"><i class="fas fa-chevron-down"></i></span>
                        </div>
                    ...
                    <div>
                        <label class="mb-2 block font-bold text-sm text-slate-700">Initial Status</label>
                        <div class="relative">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="follow_up_date" value="{{ old('follow_up_date') }}" class="date-input w-full rounded-xl border border-gray-200 bg-gray-50 py-3 px-5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white">
            ...
            <div class="border-t border-gray-100 pt-6 flex justify-end gap-3">
                ...
                <button type="submit" class="rounded-xl bg-blue-600 py-3 px-8 font-bold text-white hover:bg-blue-700 transition shadow-md">Save Inquiry</button>
            </div>
        </form>
    </div>
</div>
